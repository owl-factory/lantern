...
interface TableComponentProps<T, U> {
  data: T,
  globalData?: U;
}
export type TableComponent<T, U> = (props: TableComponentProps<T, U>) => (JSX.Element);

export type TableComponentModification = (value: string | boolean) => string;

export interface Column<T, U> {
  ...
  component?: TableComponent<T, U>; // A custom component to print using the row data
