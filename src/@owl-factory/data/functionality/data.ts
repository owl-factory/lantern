 ...
 */
export function get<T extends GenericRecord>(this: DataManager<T>, ref: string): T | undefined {
 ...
 */
export async function load(this: DataManager<GenericRecord>, ref: string | string[], reloadPolicy: ReloadPolicy = this.reloadPolicy): Promise<boolean> {
 ...
 * @param loaded If these are the full document that a user can get -- eg, it is fully loaded. If not, false. This applies to all; if one document 
 * is not fully loaded, either exclude and run with set or a seperate setMany call, or mark all as false. Loaded documents can be marked as unloaded,
 * but unloaded documents can never be marked loaded.
 */
