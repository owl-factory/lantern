import { getGlobalPermissionKeys, getGlobalRole } from "./globals";
...
export function permissionsFromBinary(binary: string, globalPermissions?: string[]) {
  ...
  if (binary.length - 1 !== globalPermissions.length) {
    ...
    console.error(`Permission Conversion Exception: A mismatch has occured between the permission binary and global permissions lengths. No permissions are granted.`);
    return [];
  }
 ...
 * @todo This code cannot rely on an external Auth, as it causes a circular loop
 */
