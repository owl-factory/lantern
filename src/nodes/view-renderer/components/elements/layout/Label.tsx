 ...
 */
export const ViewLabel = observer((props: RenderProps<LabelAttributes>) => {
  const sources = ViewRenderer.renders[props.renderID].sources;

  const [ className, setClassName ] = React.useState("");
  const [ htmlFor, setHTMLFor ] = React.useState("");
  ...
  // ID
  React.useEffect(() => {
    runExpression(sources, props.element.attributes.for, props.properties).then((res: string) => {
      setHTMLFor(res);
    });
