import { PageMetadata } from "nodes/view-renderer/types/pages";
...
export function parsePageGroups(xmlDOM: XMLDocument): Record<string, any> {
  const pageGroups: Record<string, PageMetadata[]> = {};
  ...
  for (const pageable of pageables) {
    ...
    for (const page of pageable.children) {
      ...
      pages.push({ name: pageName}); // This struct will eventually contain any additional information for who can view
    }
